...
function HeadingLevelConfig({
  label, description, prefix, nameExample,
  content, onContentChange, numberStyle, onNumberStyleChange,
}: {
  label: string;
  description?: string;
  prefix: string;
  ...
  return (
    <Stack gap="xs">
      <div>
        ...
        <Select
          data={headingContentOptions(prefix, nameExample)}
          value={content}
...
function EpubPreview({
  ...
  return (
    <div style={{
      ...
      {/* Book title strip (only when multiple boards) */}
      {showBookTitle && (
        <div style={{
          textAlign: 'center',
          paddingBottom: 18,
...
export default function EpubExportForm({ projectId, projectTitle, boards, defaultBoardId }: Props) {
  // Board selection: if a specific board was linked, pre-select only that one;
  // otherwise select all boards by default.
  const [selectedBoardIds, setSelectedBoardIds] = useState<number[]>(
    defaultBoardId !== null && boards.some((b) => b.id === defaultBoardId)
      ? [defaultBoardId]
      : boards.map((b) => b.id),
  );
  ...
  const [tocDepth, setTocDepth] = useState<1 | 2 | 3>(selectedBoardIds.length > 1 ? 2 : 2);

  // UI state
  const [loading, setLoading] = useState<'epub' | 'docx' | null>(null);
  const [error, setError] = useState<string | null>(null);

  const multiBook = selectedBoardIds.length > 1;

  function toggleBoard(id: number, checked: boolean) {
    setSelectedBoardIds((prev) =>
      checked ? [...prev, id] : prev.filter((x) => x !== id),
    );
  }

  function buildSettings(): EpubSettings {
    return {
  ...
  async function handleDownload(format: 'epub' | 'docx') {
    if (!title.trim()) { setError('Title is required.'); return; }
    if (selectedBoardIds.length === 0) { setError('Select at least one board to export.'); return; }
    setError(null);
    setLoading(format);

    const route = format === 'epub'
      ? `/writing/${projectId}/export/epub/download`
      : `/writing/${projectId}/export/docx/download`;

    try {
      const res = await fetch(route, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ boardIds: selectedBoardIds, settings: buildSettings() }),
      });
      if (!res.ok) {
      ...
      a.href = url;
      a.download = `${title.trim().replace(/[^a-z0-9\- ]/gi, '_')}.${format}`;
    ...
    <Stack gap="md" style={{ maxWidth: 600 }}>
      ...
      >
        ...
        <Accordion.Item value="books">
          <Accordion.Control>
            <Group gap="xs">
              <Text fw={600} size="sm">Books</Text>
          ...
          <Accordion.Panel>
            <Text size="sm" c="dimmed" mb="sm">
              Each selected board becomes a titled section in the epub,
              compiled in order. Boards with no compiled cards are skipped.
            </Text>
            <Stack gap="xs">
              {boards.map((board, i) => (
                <Paper key={board.id} withBorder p="sm" style={{
                  opacity: selectedBoardIds.includes(board.id) ? 1 : 0.5,
                }}>
                  <Group gap="sm">
                    <Checkbox
                      checked={selectedBoardIds.includes(board.id)}
                      onChange={(e) => toggleBoard(board.id, e.currentTarget.checked)}
                      color="dark"
                    />
                    <div style={{ flex: 1 }}>
            ...
            {boards.length > 1 && (
              <Group gap="xs" mt="sm">
                <Button size="xs" variant="subtle" color="dark"
                  onClick={() => setSelectedBoardIds(boards.map((b) => b.id))}>
                  Select all
                </Button>
              ...
              >
                <Group gap="sm" align="flex-start">
                  ...
                  <div>
                    ...
                    <Group gap={4} mt="xs">
                      {multiBook && <Badge size="xs" variant="light" color="violet">Boards → Books</Badge>}
                ...
                <div>
                  ...
                  <SegmentedControl
                    data={
                      multiBook
                        ? [
                            { value: '1', label: 'Books only' },
                            { value: '2', label: 'Books + parts' },
                            { value: '3', label: 'Full depth' },
                          ]
                        : [
                    ...
                    onChange={(v) => setTocDepth(Number(v) as 1 | 2 | 3)}
                    color="dark"
                  />
                </div>
              )}
      ...
      <Box>
        ...
        <Group grow gap="sm">
          <Button
            ...
            color="dark"
            variant="filled"
            leftSection={loading === 'epub' ? <Loader size={16} color="white" /> : <IconDownload size={18} />}
            onClick={() => handleDownload('epub')}
            disabled={loading !== null || selectedBoardIds.length === 0}
          >
            {loading === 'epub' ? 'Generating…' : '.epub'}
          </Button>
          <Button
            size="md"
            color="blue"
            variant="light"
            leftSection={loading === 'docx' ? <Loader size={16} color="white" /> : <IconFileWord size={18} />}
            onClick={() => handleDownload('docx')}
            disabled={loading !== null || selectedBoardIds.length === 0}
          >
            {loading === 'docx' ? 'Generating…' : '.docx'}
          </Button>
        </Group>
        {multiBook && (
